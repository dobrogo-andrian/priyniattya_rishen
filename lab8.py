...
class DecisionTree:
    ...
    def draw_tree(self, filename: str = None, figsize=(16, 10), ylim=(-1, 11)):
        """Намалювати дерево рішень з налаштовуваними розмірами"""
        fig, ax = plt.subplots(1, 1, figsize=figsize)
        ax.set_xlim(-0.5, 11)
        ax.set_ylim(ylim)
        ax.axis('off')

        ax.text(5.5, ylim[1] - 0.5, self.title, fontsize=14, weight='bold', ha='center')
...
def task1_create_production_tree(A1: float, A2: float, A3: float,
    ...
    tree.draw_tree("task1_decision_tree.png", figsize=(14, 8), ylim=(-0.5, 10.5))
...
def task2_market_research_tree(A1: float, A2: float, A3: float,
    ...
    print(
        f"      P(Факт Неспр. | Прогноз Спр.) = ({P12}×{p_unfavorable})/{p_prog_fav:.3f} = {p_unfav_given_prog_fav:.3f}")
    print(f"   При несприятливому прогнозі:")
    print(
        f"      P(Факт Спр. | Прогноз Неспр.) = ({P21}×{p_favorable})/{p_prog_unfav:.3f} = {p_fav_given_prog_unfav:.3f}")
    print(
        f"      P(Факт Неспр. | Прогноз Неспр.) = ({P22}×{p_unfavorable})/{p_prog_unfav:.3f} = {p_unfav_given_prog_unfav:.3f}")
    ...
    print(
        f"      Велике: EV = {p_fav_given_prog_fav:.3f}×{A1:,.0f} + {p_unfav_given_prog_fav:.3f}×{B1:,.0f} = {ev1_prog_fav:,.0f} г.о.")
    print(
        f"      Мале:   EV = {p_fav_given_prog_fav:.3f}×{A2:,.0f} + {p_unfav_given_prog_fav:.3f}×{B2:,.0f} = {ev2_prog_fav:,.0f} г.о.")
    print(
        f"      Патент: EV = {p_fav_given_prog_fav:.3f}×{A3:,.0f} + {p_unfav_given_prog_fav:.3f}×{B3:,.0f} = {ev3_prog_fav:,.0f} г.о.")
    ...
    print(
        f"      Велике: EV = {p_fav_given_prog_unfav:.3f}×{A1:,.0f} + {p_unfav_given_prog_unfav:.3f}×{B1:,.0f} = {ev1_prog_unfav:,.0f} г.о.")
    print(
        f"      Мале:   EV = {p_fav_given_prog_unfav:.3f}×{A2:,.0f} + {p_unfav_given_prog_unfav:.3f}×{B2:,.0f} = {ev2_prog_unfav:,.0f} г.о.")
    print(
        f"      Патент: EV = {p_fav_given_prog_unfav:.3f}×{A3:,.0f} + {p_unfav_given_prog_unfav:.3f}×{B3:,.0f} = {ev3_prog_unfav:,.0f} г.о.")
    ...
    chance_no1 = ChanceNode("Ринок", 5.5, 10)
    chance_no2 = ChanceNode("Ринок", 5.5, 8)
    chance_no3 = ChanceNode("Ринок", 5.5, 6)

    end_no11 = EndNode("С", 8, 10.8, A1)
    end_no12 = EndNode("Н", 8, 9.2, B1)
    end_no21 = EndNode("С", 8, 8.8, A2)
    end_no22 = EndNode("Н", 8, 7.2, B2)
    end_no31 = EndNode("С", 8, 6.8, A3)
    end_no32 = EndNode("Н", 8, 5.2, B3)
    ...
    decision_no_research.add_child(chance_no3, "Продати патент")
    ...
    decision_prog_fav.add_child(chance_fav3, "Продати патент")

    end_fav11 = EndNode("С", 10.5, 6.3, A1)
    end_fav12 = EndNode("Н", 10.5, 4.7, B1)
    end_fav21 = EndNode("С", 10.5, 4.8, A2)
    end_fav22 = EndNode("Н", 10.5, 3.2, B2)
    end_fav31 = EndNode("С", 10.5, 3.3, A3)
    end_fav32 = EndNode("Н", 10.5, 1.7, B3)
    ...
    chance_unfav1 = ChanceNode("Ринок", 8, 1)
    chance_unfav2 = ChanceNode("Ринок", 8, -0.5)
    chance_unfav3 = ChanceNode("Ринок", 8, -2)

    decision_prog_unfav.add_child(chance_unfav1, "Велике")
    decision_prog_unfav.add_child(chance_unfav2, "Мале")
    decision_prog_unfav.add_child(chance_unfav3, "Продати патент")

    end_unfav11 = EndNode("С", 10.5, 1.8, A1)
    end_unfav12 = EndNode("Н", 10.5, 0.2, B1)
    end_unfav21 = EndNode("С", 10.5, 0.3, A2)
    end_unfav22 = EndNode("Н", 10.5, -1.3, B2)
    end_unfav31 = EndNode("С", 10.5, -1.2, A3)
    end_unfav32 = EndNode("Н", 10.5, -2.8, B3)
    ...
    tree.draw_tree("task2_decision_tree.png", figsize=(18, 16), ylim=(-3.5, 12))

    return tree, evpi


def task3_supplier_selection_tree(prob_A: List[float], prob_B: List[float], K: float, N: int, L: float):
    """
...
""")
    ...
    if evpi > 0:
        ...
        print(f"   перевищує вартість дослідження ({Q:,.0f} г.о.)\n")
    else:
